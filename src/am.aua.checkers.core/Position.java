...
public class Position {
     ...
     * @return
     */
    public static ArrayList<Position> appendPositionsToArray(ArrayList<Position> arr, ArrayList<Position> elements) {
        ArrayList<Position> result = new ArrayList<>(arr.size() + elements.size());
        for (int i = 0;i< arr.size();i++){
            result.add(i, arr.get(i));
        }
        for (int j = 0;j< elements.size();j++){
            result.add(arr.size()+j, elements.get(j));
        }
        return result;
    }
